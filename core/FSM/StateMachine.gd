#const State = preload("res://core/FSM/State.gd")
...
func set_target(target):
	...
	"""
	_target = target
	for s in states.values(): s.set_target(target)
...
func set_state(state_id, state):
	...
	if _target: state.set_target(_target)
...
func onGoto(obj):
	if _current_state.has_method("onGoto"): _current_state.onGoto(obj)

func onInteract(obj):
	if _current_state.has_method("onInteract"): _current_state.onInteract(obj)
